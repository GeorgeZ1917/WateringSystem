...
/* This library was created by Preston Sundar, and is available at GitHub (https://github.com/Preston-Sundar/DS3231-RTC-AVR-Atmega-Library). */
...
void DS3231_getTime ( uint8_t DateArray [] ) {
	...
	DateArray [ 0 ] = bcdToDec(twi_readChar_ACK() & 0x7f);
	DateArray [ 1 ] = bcdToDec(twi_readChar_ACK());	
	DateArray [ 2 ] = bcdToDec(twi_readChar_ACK() & 0x3f);
	DateArray [ 3 ] = bcdToDec(twi_readChar_ACK());
	DateArray [ 4 ] = bcdToDec(twi_readChar_ACK());
	DateArray [ 5 ] = bcdToDec(twi_readChar_ACK());
	DateArray [ 6 ] = bcdToDec(twi_readChar_ACK());
}
